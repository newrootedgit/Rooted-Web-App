...
export const onboardingRouter = router({
    status: authedProcedure.query(({ ctx }) => getOnboardingStatus(ctx.prisma, ctx.userId)),
    createTenantAndFarm: authedProcedure
        .input(createTenantAndFarmSchema)
        .mutation(({ ctx, input }) => createTenantAndFarm(ctx.prisma, ctx.userId, input)),
    farms: authedProcedure.query(({ ctx }) => getUserFarms(ctx.prisma, ctx.userId)),
});
