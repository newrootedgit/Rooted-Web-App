...
describe('createOrUpdateMachine', () => {
    ...
    it('should create new machine when deviceId does not exist', async () => {
        ...
        const result = await createOrUpdateMachine(mockPrisma, { name: 'New Machine', deviceId: 'new-device' }, 'tenant-1', 'farm-1');
    ...
    it('should update existing machine when deviceId exists', async () => {
        ...
        const result = await createOrUpdateMachine(mockPrisma, { name: 'Updated Name', deviceId: 'existing-device' }, 'tenant-2', 'farm-2');
    ...
    it('should associate machine with correct tenant and farm', async () => {
        ...
        mockPrisma.machines.create.mockResolvedValue(createdMachine);
        const result = await createOrUpdateMachine(mockPrisma, { name: 'Test', deviceId: 'dev-1' }, 'specific-tenant', 'specific-farm');
    ...
    it('should create machine with null farmId', async () => {
        ...
        mockPrisma.machines.create.mockResolvedValue(createdMachine);
        const result = await createOrUpdateMachine(mockPrisma, { name: 'No Farm Machine', deviceId: 'device-123' }, 'tenant-1', null);
        expect(mockPrisma.machines.create).toHaveBeenCalledWith({
            data: {
                tenant_id: 'tenant-1',
                farm_id: null,
    ...
    it('should return properly formatted Machine object', async () => {
        ...
        const result = await createOrUpdateMachine(mockPrisma, { name: 'Formatted Machine', deviceId: 'device-456' }, 'tenant-abc', 'farm-xyz');
...
//# sourceMappingURL=createOrUpdateMachine.test.js.map